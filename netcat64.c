...
#define SA6 struct sockaddr_in6  /* socket overgeneralization braindeath */
#define SAI6 struct sockaddr_in6 /* ... whoever came up with this model */
//#define IA struct in6_addr     /* ... should be taken out and shot, */
...
struct host_poop {
  ...
  char addrs[8][50];                  /* ascii-format IP addresses */
  struct sockaddr_storage ssaddrs[8]; /* real addresses: in_addr */
};                                    /* real addresses: in6_addr */
...
SAI * remend = NULL;
SAI6 * lclend6 = NULL;        /* sockaddr_in structs */
SAI6 * remend6 = NULL;
#ifdef MULTICAST
struct in_addr * mgroup = NULL;
struct in6_addr * mgroup6 = NULL;
#ifdef WIN32
...
char* FAR inet6_ntoa( struct in6_addr in)
{
    ...
    tmp.sin6_addr=in;
    //tmp.sin6_port=0xFFFF;
    if (WSAAddressToString((struct sockaddr *)&tmp, sizeof(struct sockaddr_in6), 0, buffer, &len) != 0)
    {
        //int qwe=WSAGetLastError();
        return "WSAAddressToString error";
    }
    return buffer;
}

struct in6_addr any_in6 = IN6ADDR_ANY_INIT;

struct in6_addr inet6_addr(const char FAR* cp)
{
    DWORD len = sizeof(struct sockaddr_in6);
    //undigned long qwe = inet_addr("localhost");
    struct in6_addr result;
    struct sockaddr_in6 tmp;

    memset(&tmp, 0, sizeof(tmp));
    tmp.sin6_family = AF_INET6;
    if (WSAStringToAddress(cp, AF_INET6, 0, &tmp, &len) != 0)
    {
        //int qwe=WSAGetLastError();
        IN6_SET_ADDR_UNSPECIFIED(&result);
        return result;
    }
    result=tmp.sin6_addr;
    return result;
}
...
#ifdef ANAL            
    if (strcmp (poop->name, hp) != 0)         /* case-sensitive */
#else
    if (strcasecmp (poop->name, hp) != 0)     /* normal */
#endif
    {
        holler ("DNS fwd/rev mismatch: %s != %s", poop->name, hp);
        return (1);
    }
    return (0);
...
/* see wzv:workarounds.c for dg/ux return-a-struct inet_addr lossage */
    ...
    /* success or don't use DNS = done */
    if (status == 0 || numeric)
    {
        if (status != 0)
        {
            freeaddrinfo(res);
            bail ("Can't parse `%s' as an address - [%d]", name, WSAGetLastError());
        }
...
/*            addr = &(ipv4->sin_addr); */
            sa4 = (struct sockaddr_in *)&poop->ssaddrs[x];
            sa4->sin_family = res->ai_family;
            sa4->sin_addr = ipv4->sin_addr;
...
/*            addr = &(ipv6->sin6_addr); */
            sa6 = (struct sockaddr_in6 *)&poop->ssaddrs[x];
            sa6->sin6_family = res->ai_family;
            sa6->sin6_addr = ipv6->sin6_addr;
            strcpy (ipstr, inet6_ntoa(ipv6->sin6_addr));
        }

/*        inet_ntop(res->ai_family, addr, (PSTR)ipstr, sizeof(ipstr)); */
        strncpy(poop->addrs[x], ipstr, strlen(ipstr));
        ...
        if (o_verbose)
        {
            ...
            status = getnameinfo(((res->ai_family == AF_INET) ? sa4 : sa6), res->ai_addrlen, node, sizeof(node), NULL, 0, NI_NAMEREQD);
            if (status)
                holler ("Warning: reverse host lookup failed for `%s' - [%d]", poop->addrs[x], WSAGetLastError());
            else
            {
                strncpy (poop->name, node, MAXHOSTNAMELEN - 2);
...
/*                hints.ai_socktype = SOCK_STREAM; */
                ...
                if (getaddrinfo(poop->name, NULL, &hints, &res))
                    holler ("Warning: forward host lookup failed for `%s' - [%d]", poop->name, WSAGetLastError());
                else
                   (void) comparehosts (poop, node);
            }
        }
    }
    else
    {
        /* failure = not numeric - DNS lookup */
        memset(&res, 0, sizeof res);
        hints.ai_flags &= ~AI_NUMERICHOST;
        hints.ai_flags = AI_CANONNAME|AI_PASSIVE;
/*        hints.ai_socktype = SOCK_STREAM; */
        ...
        {
            freeaddrinfo(res);
            bail ("forward host lookup failed for `%s' - [%d]", name, WSAGetLastError());
        }

        strncpy (poop->name, res->ai_canonname, sizeof (poop->name));
...
/*            inet_ntop(p->ai_family, addr, (PSTR)ipstr, sizeof(ipstr)); */
            ...
            {
                char node[NI_MAXHOST];
                status = getnameinfo(p->ai_addr, p->ai_addrlen, node, sizeof(node), NULL, 0, NI_NAMEREQD);
                if (status)
                    holler ("Warning: reverse host lookup failed for `%s' - [%d]", poop->addrs[x], WSAGetLastError());
                else
                    (void) comparehosts (poop, node);
...
/* whatever-all went down previously, we should now have a host_poop struct
   with at least one IP address in it. */
    ...
    /* determine v4 or v6 and set globally */
    if (!(gv4 || gv6))
    {
Debug (("Address Family = %d", poop->ssaddrs[0].ss_family))

        if (poop->ssaddrs[0].ss_family == AF_INET)
            gv4 = 1;
        else if (poop->ssaddrs[0].ss_family == AF_INET6)
            gv6 = 1;
        else
            bail ("Can't determine address family to use");

        if (o_verbose > 1)
            holler ("Selected %s", ((gv4) ? "IPv4" : "IPv6"));
    }
...
#define MAX_TRIES 3
/* GetAdapterPoop :
   Given a 'struct in6_addr', return the ifIndex it corresponds to.
   If not found, return ifIndex 0.
   Bail on error. */
int GetAdapterPoop (addr)
    struct in6_addr * addr;
{
    ...
    /* iteratively do the GetAdaptersAddresses() call to determine buffer size */
    do
    {
        pAddresses = (IP_ADAPTER_ADDRESSES *) HeapAlloc(GetProcessHeap(), 0, outBufLen);
        if (pAddresses == NULL)
        {
            holler ("HeapAlloc IP_ADAPTER_ADDRESSES failed - using default multicast interface");
            return IfIndex;
        }

        if ((ret = GetAdaptersAddresses(AF_INET6, flags, NULL, pAddresses, &outBufLen)) == ERROR_BUFFER_OVERFLOW)
        {
    ...
    /* pAddresses is the pointer once the call succeeds */
    if (ret == NO_ERROR)
    {
...
Debug (("ifIndex = %ld => \t%s", pCurrAddresses->Ipv6IfIndex, inet6_ntoa(((struct sockaddr_in6 *)pUnicast->Address.lpSockaddr)->sin6_addr)));

                /* compare returned 'struct in6_addr' with passed in value */
                if (memcmp(&((struct sockaddr_in6 *)pUnicast->Address.lpSockaddr)->sin6_addr, addr, sizeof (struct in6_addr)) == 0)
                {
                    IfIndex = pCurrAddresses->Ipv6IfIndex;
                    goto gap_done;
                }
        ...
        holler ("GetAdaptersAddresses() failed - using default multicast interface");
...
/* grab a socket; set opts */
    ...
    if (gSOttl > -1)
    {
        if (setsockopt(nnetfd, IPPROTO_IPV6, IPV6_UNICAST_HOPS, (char *)&gSOttl, sizeof(char)) == SOCKET_ERROR)
            holler("setsockopt(IPV6_UNICAST_HOPS) failed - [%d]", WSAGetLastError());

#ifdef MULTICAST
        if (setsockopt(nnetfd, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, (char *)&gSOttl, sizeof(char)) == SOCKET_ERROR)
            holler("setsockopt(IPV6_MULTICAST_HOPS) failed - [%d]", WSAGetLastError());
#endif
...
#ifdef SSODEBUG
    if (gSOdebug)
        if (setsockopt(nnetfd, SOL_SOCKET, SO_DEBUG, (char *)&gSOdebug, sizeof(int)) == SOCKET_ERROR)
            holler("setsockopt(SO_DEBUG) failed - [%d]", WSAGetLastError());
#endif
#ifdef SSOKEEPALIVE
    if (!o_udpmode)
    {
        ...
        {
            DWORD nBytes=0;
            if (WSAIoctl(nnetfd, SIO_KEEPALIVE_VALS, &alive, sizeof(alive), NULL, 0, &nBytes, NULL, NULL ) != 0)
                holler("WSAIoctl(SIO_KEEPALIVE_VALS) failed - [%d]", WSAGetLastError());
            if (setsockopt(nnetfd, SOL_SOCKET, SO_KEEPALIVE, (char *)&gSOkeepalive, sizeof(int)) == SOCKET_ERROR)
                holler("setsockopt(SO_KEEPALIVE) failed - [%d]", WSAGetLastError());
        }
    }
#endif
/* no SSOBC for IPv6 */
    ...
    if (mgroup6)
    {
        ...
        if (rad)
        {
            ...
            /* not as easy as
             * mreq.gsr_group = *mgroup6;
             */
            struct sockaddr_in6 g6;
            g6.sin6_addr = *mgroup6;
            memcpy(&mreq.gsr_group, &g6, sizeof(struct sockaddr_in6));

            /* _interface member is of ifIndex, not IP[v6] address like the ip_mreq structure */
#ifdef WIN32
            if (lad)
                mreq.gsr_interface = GetAdapterPoop(lad);
            else
#endif
            ...
            if (setsockopt(nnetfd, IPPROTO_IPV6, MCAST_JOIN_SOURCE_GROUP, (char *)&mreq, sizeof(GROUP_SOURCE_REQ)) == SOCKET_ERROR)
                bail("setsockopt(MCAST_JOIN_SOURCE_GROUP) failed - [%d]", WSAGetLastError());
        }
        else
        {
#endif /* IPv6SSM */
            struct ipv6_mreq mreq;
            mreq.ipv6mr_multiaddr = *mgroup6;
            /* _interface member is of ifIndex, not IP[v6] address like the ip_mreq structure */
#ifdef WIN32
            if (lad)
                mreq.ipv6mr_interface = GetAdapterPoop(lad);
            else
#endif
                mreq.ipv6mr_interface = 0;

            if (setsockopt(nnetfd, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, (char *)&mreq, sizeof(struct ipv6_mreq)) == SOCKET_ERROR)
                bail("setsockopt(IPV6_ADD_MEMBERSHIP) failed - [%d]", WSAGetLastError());
#ifdef IPv6SSM
        }
#endif
    }
...
/* if lad/lp, do appropriate binding */
    if (lad)
        memcpy (&lclend6->sin6_addr/*.s_addr*/, lad, sizeof (struct in6_addr));
    if (lp)
...
/* try a few times for the local bind, a la ftp-data-port... */
        for (y = 4; y > 0; y--)
        {
            rr = bind (nnetfd, (SA6 *)lclend6, sizeof (SA6));
            if (rr == 0)
                break;
            if (errno != EADDRINUSE)
                break;
            ...
            {
                holler ("retrying local %s:%d", inet6_ntoa (lclend6->sin6_addr), lp);
                sleep (1);
    ...
    inet6_ntoa(lclend6->sin6_addr), lp);

    if (o_listen)
        return (nnetfd);            /* thanks, that's all for today */

    memcpy (&remend6->sin6_addr/*.s_addr*/, rad, sizeof (struct in6_addr));
...
/* #ifdef IP_OPTIONS */
        ...
        if (! optbuf)        /* and don't already *have* a srcrt set */
        {
            ...
            *opp++ = (char)
            (((gatesidx + 1) * sizeof (struct in6_addr)) + 3) & 0xff; /* length */
            *opp++ = gatesptr;                                        /* pointer */
/* opp now points at first hop addr -- insert the intermediate gateways */
            for ( x = 0; x < gatesidx; x++)
            {
                memcpy (opp, gates[x]->iaddrs, sizeof (struct in6_addr));
                opp += sizeof (struct in6_addr);
            }
/* and tack the final destination on the end [needed!] */
            memcpy (opp, rad, sizeof (struct in6_addr));
            opp += sizeof (struct in6_addr);
...
/* calculate length of whole option mess, which is (3 + [hops] + [final] + 1),
   and apply it [have to do this every time through, of course] */
        x = ((gatesidx + 1) * sizeof (struct in6_addr)) + 4;
...
/* wrap connect inside a timer, and hit it */
    arm (1, o_wait);
    if (setjmp (jbuf) == 0)
        rr = connect (nnetfd, (SA6 *)remend6, sizeof (SA6));
    else
...
/* grab a socket; set opts */
    ...
    if (gSOttl > -1)
    {
        if (setsockopt(nnetfd, IPPROTO_IP, IP_TTL, (char *)&gSOttl, sizeof(char)) == SOCKET_ERROR)
            holler("setsockopt(IP_TTL) failed - [%d]", WSAGetLastError());

#ifdef MULTICAST
        if (setsockopt(nnetfd, IPPROTO_IP, IP_MULTICAST_TTL, (char *)&gSOttl, sizeof(char)) == SOCKET_ERROR)
            holler("setsockopt(IP_MULTICAST_TTL) failed - [%d]", WSAGetLastError());
#endif
    ...
    if (gSObc)
    {
        if (setsockopt(nnetfd, SOL_SOCKET, SO_BROADCAST, (char *)&gSObc, sizeof(int)) == SOCKET_ERROR)
            holler("setsockopt(SO_BROADCAST) failed - [%d]", WSAGetLastError());
        if (o_listen)
            if (setsockopt(nnetfd, IPPROTO_IP, IP_RECEIVE_BROADCAST, (char *)&gSObc, sizeof(int)) == SOCKET_ERROR)
                holler("setsockopt(IP_RECEIVE_BROADCAST) failed - [%d]", WSAGetLastError());
    }
#endif
#ifdef MULTICAST
    if (mgroup)
    {
        ...
        if (rad)
        {
            ...
            if (lad)
                mreq.imr_interface = *lad;
            else
                mreq.imr_interface.s_addr = htonl(INADDR_ANY);
            mreq.imr_sourceaddr = *rad;

            if (setsockopt(nnetfd, IPPROTO_IP, IP_ADD_SOURCE_MEMBERSHIP, (char *)&mreq, sizeof(struct ip_mreq_source)) == SOCKET_ERROR)
                bail("setsockopt(IP_ADD_SOURCE_MEMBERSHIP) failed - [%d]", WSAGetLastError());
        }
        else
        {
            struct ip_mreq mreq;
            ...
            if (setsockopt(nnetfd, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char *)&mreq, sizeof(struct ip_mreq)) == SOCKET_ERROR)
                bail("setsockopt(IP_ADD_MEMBERSHIP) failed - [%d]", WSAGetLastError());
        }
    }
#endif
...
/* I can't believe I have to do all this to get my own goddamn bound address
   ...
   random unknown port is probably not very useful without "netstat". */
    ...
    if (o_verbose)
    {
        x = sizeof (SA6);        /* how 'bout getsockNUM instead, pinheads?! */
        rr = getsockname (nnetfd, (SA6 *) lclend6, &x);
        if (rr < 0)
            holler ("local getsockname failed");
        strcpy (bigbuf_net, "listening on [");    /* buffer reuse... */
        if (memcmp (&lclend6->sin6_addr/*.s_addr*/, &any_in6,sizeof(struct in6_addr) ))
            strcat (bigbuf_net, inet6_ntoa (lclend6->sin6_addr));
        else
            strcat (bigbuf_net, "::");
        strcat (bigbuf_net, "] %d (%s) ...");
...
#ifdef IPv6SSM
            if (rad)
                strcat (bigbuf_net, inet6_ntoa(*rad));
            else
#endif
                strcat (bigbuf_net, "*");
            strcat (bigbuf_net, ",");
            strcat (bigbuf_net, inet6_ntoa(*mgroup6));
            strcat (bigbuf_net, "}");
        }
#endif

        z = ntohs (lclend6->sin6_port);
        holler (bigbuf_net, z, (o_udpmode) ? "UDP" : "TCP");
    } /* verbose -- whew!! */
...
/* UDP is a speeeeecial case -- we have to do I/O *and* get the calling
   ...
   actually does work after all.  Yow.  YMMV on strange platforms!  */
    if (o_udpmode)
    {
        x = sizeof (SA6);       /* retval for recvfrom */
        arm (2, o_wait);        /* might as well timeout this, too */
        if (setjmp (jbuf) == 0) /* do timeout for initial connect */
        {
            rr = recvfrom        /* and here we block... */
            (nnetfd, bigbuf_net, BIGSIZ, MSG_PEEK, (SA6 *) remend6, &x);

Debug (("dolisten/recvfrom ding, rr = %d, netbuf %s ", rr, bigbuf_net))

        }
        else
            goto dol_tmo6;        /* timeout */
...
*/
#if defined SSOBC && defined MULTICAST
        if (!gSObc && !mgroup6)
#elif SSOBC
        if (!gSObc)
#elif MULTICAST
...
/* fall here for TCP */
    x = sizeof (SA6);        /* retval for accept */
    arm (2, o_wait);        /* wrap this in a timer, too; 0 = forever */
    if (setjmp (jbuf) == 0)
        rr = accept (nnetfd, (SA6 *)remend6, &x);
    else
        goto dol_tmo6;        /* timeout */
...
whoisit6:
    ...
    if (rr < 0)
        goto dol_err6;        /* bail out if any errors so far */
...
/* find out what address the connection was *to* on our end, in case we're
   ...
   "virtual web site" hack. */
    ...
    /* need to make buffer larger and move further in for IPv6 as addresses are longer */
    memset (bigbuf_net, 0, 96); /* original = 64 */
    cp = &bigbuf_net[48];       /* original = 32 */
    x = sizeof (SA6);
    rr = getsockname (nnetfd, (SA6 *) lclend6, &x);
    if (rr < 0)
        holler ("post-rcv getsockname failed");
    strcpy (cp, inet6_ntoa (lclend6->sin6_addr));
...
/* now check out who it is.  We don't care about mismatched DNS names here,
   ...
   accept the connection and then reject undesireable ones by closing. */
    ...
    strcpy (bigbuf_net, inet6_ntoa (remend6->sin6_addr));
    whozis = gethostpoop (bigbuf_net, o_nflag);
    errno = 0;
    x = 0;                /* use as a flag... */

    struct sockaddr_in6 * sa6;
    sa6 = (struct sockaddr_in6 *)&whozis->ssaddrs[x];

    if (rad)
/*        if (memcmp (rad, whozis->ssaddrs, sizeof (SA6))) */
        if (memcmp (rad, &(sa6->sin6_addr), sizeof (struct in6_addr)))
            x = 1;
    if (rp)
        if (z != rp)
            x = 1;
...
#ifdef FIXINVALCONN
    {
        holler ("invalid connection to [%s] from %s [%s] %d", cp, whozis->name, whozis->addrs[0], z);
#ifdef WIN32
        shutdown(nnetfd, 0x02);  /* Kirby */
        closesocket (nnetfd);
#else
   ...
   random unknown port is probably not very useful without "netstat". */
    ...
    if (o_verbose)
    {
        ...
        if (mgroup)
        {
            strcat (bigbuf_net, " {");
            if (rad)
                strcat (bigbuf_net, inet_ntoa(*rad));
            else
                strcat (bigbuf_net, "*");
            strcat (bigbuf_net, ",");
            strcat (bigbuf_net, inet_ntoa(*mgroup));
            strcat (bigbuf_net, "}");
        }
#endif
...
/* now check out who it is.  We don't care about mismatched DNS names here,
   ...
   accept the connection and then reject undesireable ones by closing. */
    ...
    struct sockaddr_in * sa4;
    sa4 = (struct sockaddr_in *)&whozis->ssaddrs[x];

    if (rad)
/*        if (memcmp (rad, whozis->ssaddrs, sizeof (SA))) */
        if (memcmp (rad, &(sa4->sin_addr), sizeof (struct in_addr)))
            x = 1;
    if (rp)
        if (z != rp)
            x = 1;
...
//  IA * themaddr = NULL;
    ...
    struct in6_addr * ouraddr6 = NULL;
    struct in6_addr * themaddr6 = NULL;
    USHORT o_lport = 0;
    USHORT ourport = 0;
...
/* round up the usual suspects, i.e. malloc up all the stuff we need */
    ...
    lclend6 = (SAI6 *) Hmalloc (sizeof (SA6));
    remend6 = (SAI6 *) Hmalloc (sizeof (SA6));
...
#ifdef FIXRELISTENHOST
    /*
     * If re-listening, don't want optind to increase otherwise any remote addr/port
     * pair for lock-down listen gets corrupted and fails.  Since optind will increase 
     * below with the (themaddr || themaddr6) check, we'll just pre-decrement here 
     * only if listen is also indicated so that it stays the same.
     */
    if (o_listen && (themaddr || themaddr6))
        optind--;
#endif
...
/* optarg, optind = next-argv-component [i.e. flag arg]; optopt = last-char */
    char arglist[50] = "46adg:G:hi:klLno:p:rs:uvw:z";
...
#endif
#endif
    while ((x = getopt (argc, argv, arglist)) != EOF)
    {
        ...
        switch (x)
        {
            case '4':
                if (!gv6)
                    ...
                    bail("-4 and -6 mutually exclusive");
                break;
            ...
            case '6':
                if (!gv4)
                    ...
                    bail("-6 and -4 mutually exclusive");
                break;
            ...
            case 'L':                /* listen then cycle back to start instead of exiting */
                o_listen++; 
                                     /* no break, drop through to add cycle */
            case 'k':
                cycle = 1;
                break;
...
#ifdef SSOKEEPALIVE
            case 'K':                /* SO_KEEPALIVE */
                if (((gSOkeepalive = atoi(optarg)) < 1) || (gSOkeepalive > 7200))
                    bail ("Preposterous keepalive `%s': 1<=keepalive<=7200", optarg);
                alive.onoff = 1;
                alive.keepaliveinterval = 1000; /* interval of a probe when failed, in ms */
                alive.keepalivetime = gSOkeepalive * 1000; /* every X ms, default is 2 hours */
                gSOkeepalive = 1;
                break;
#endif
...
#ifdef MULTICAST
            case 'j':                /* multicast listen mode */
                whatgroup = gethostpoop (optarg, o_nflag);
                if (gv4)
                {
                    struct sockaddr_in * sa4;
                    sa4 = (struct sockaddr_in *)&whatgroup->ssaddrs[0];
                    mgroup = &(sa4->sin_addr);
                }
                else
                {
                    struct sockaddr_in6 * sa6;
                    sa6 = (struct sockaddr_in6 *)&whatgroup->ssaddrs[0];
                    mgroup6 = &(sa6->sin6_addr);
                }
                o_udpmode++;
                /* no break, drop through to set listen */
#endif
...
/* do a full lookup [since everything else goes through the same mill],
   ...
   be useful, so we'll still pass o_nflag here instead of forcing numeric.  */
                ...
                {
                    struct sockaddr_in * sa4;
                    sa4 = (struct sockaddr_in *)&wherefrom->ssaddrs[0];
                    ouraddr = &(sa4->sin_addr);
                }
                else
                {
                    struct sockaddr_in6 * sa6;
                    sa6 = (struct sockaddr_in6 *)&wherefrom->ssaddrs[0];
                    ouraddr6 = &(sa6->sin6_addr);
                }
                break;
...
#ifdef SSOTTL
            case 'T':                /* time to live */
                if (((gSOttl = atoi(optarg)) < 0) || (gSOttl > 255))
                    bail ("Preposterous TTL `%s': 0<=ttl<=255", optarg);
                break;
#endif
...
/* No broadcast in IPv6, 
   but this also controls (disables) bi-directional UDP
   allowing repeated connections from different hosts
                if (gv6)
                    bail ("-b UDPv4 only");
                gv4++;
*/
                gSObc = 1;
                /* no break, fall through to UDP */
#endif
...
/* gonna only use first addr of host-list, like our IQ was normal; if you wanna
   ...
   unless we finally implement -a, that is. */
    ...
    if (whereto && whereto->ssaddrs)
    {
        if (gv4)
        {
            struct sockaddr_in * sa4;
            sa4 = (struct sockaddr_in *)&whereto->ssaddrs[0];
            themaddr = &(sa4->sin_addr);
        }
        else
        {
            struct sockaddr_in6 * sa6;
            sa6 = (struct sockaddr_in6 *)&whereto->ssaddrs[0];
            themaddr6 = &(sa6->sin6_addr);
        }
    }
    if (themaddr || themaddr6)
...
Debug (("netfd %d from port %d to port %d", netfd, ourport, curport))
            ...
            {
                x = 0;                   /* pre-exit status */
                holler ("%s [%s] %d (%s) (%s) open",
                whereto->name, whereto->addrs[0], curport, portpoop->name, (o_udpmode) ? "UDP" : "TCP");
...
{
    o_verbose = 1;
    holler ("[v1.11.6.4 NT] Vin's World: IPv4/v6\n\
...
#else
    holler ("\
                          use [hostname] for source specific [v4 only]");
#endif
#endif
